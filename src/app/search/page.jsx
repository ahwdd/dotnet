...
import { findMaxTotal, getRates, processData, getfetchedData,
  checkLocation, getTempData, getNewestShorts, getDailyNews, getTrendingData } from "./utils/functions";
...
export default function Page({}) {
  ...
  useEffect(()=>{
    ...
    if(searchValue!=null){
      ...
      setIsLoading(true)
      getfetchedData({searchValue, perPage, currentPage, setFechedData, setNewSearchData, setStatusData, setShortsTotal,
        setTotalPages, setHasMore, setTotalAvailable, setSuggested, setIsLoading, toast, searchData, newSearchData})
    }
  }, [currentPage])
  ...
  useEffect(()=>{
    ...
    if(searchValue!=null && searchValue == initialSearchValue){
      ...
      const newLoc = `/search?s=${searchValue}&for=${searchTypeDropdownValue??''}`
  ...
  useEffect(()=>{
    console.log(searchCachValue != initialSearchValue, searchCachValue , initialSearchValue)
    if(searchCachValue && searchCachValue != initialSearchValue){
        const newLoc = `/search?s=${searchCachValue}&for=${searchTypeDropdownValue??''}`
  ...
  return (
    <main className="w-full min-h-screen overflow-hidden relative bg-white dark:bg-black">
      ...
      setPerPage={setPerPage} perPage={perPage} setCurrentPage={setCurrentPage} setInitialSearchValue={setInitialSearchValue} />
      <ToasterComponent />
      ...
      <AppBar shadow='transparent' bgTransparent={false} searchValue={searchValue??''} setSearchValue={setSearchValue} activeTabIndex={activeTabIndex} 
      setActiveTabIndex={setActiveTabIndex} searchCachValue={searchCachValue} setSearchCachValue={setSearchCachValue}
      ...
      statusData={statusData} setCurrentVid={setCurrentVid} isLoading={isLoading} setCurrentProduct={setCurrentProduct} rates={rates}
      ...
      trendingData={terndingData} tagsData={fetchedData?.tags?.data} dailyNews={dailyNews} setIsProductPopupOpen={setIsProductPopupOpen}
      currencyValue={currencyValue} setCurrencyValue={setCurrencyValue} totalPages={totalPages} setCurrentPage={setCurrentPage}
      hasMore={hasMore} total={totalAvailable} currentPage={currentPage} suggested={suggested} shortsTotal={shortsTotal} /> 
      ...
      <ProductsPlayer isPopupOpen={isProductPopupOpen} setIsPopupOpen={setIsProductPopupOpen} setCurrentProduct={setCurrentProduct} currentProduct={currentProduct} />
      <Footer />
    </main>
  );
}
