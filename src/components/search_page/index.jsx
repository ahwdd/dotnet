...
const SearchPage = ({
    ...
    (newTab, withScroll = true) => {
      ...
      if (accElement) {
        const elementPosition =
          accElement.getBoundingClientRect().top + window.scrollY;
        // const offsetPosition = elementPosition - 160;
        if (withScroll) {
          gsap.to(window, {
            duration: 0.5,
            scrollTo: { y: elementPosition, offsetY: 160 },
      ...
      if (tabWidths.length === 0) {
        const widths = data?.map(
          (_, i) => document.getElementById(`btn-${i}`)?.offsetWidth
        );
        setTabWidths(widths);
      }

      const newTabIndex = data
        ?.map((_) => _.id)
        ?.findIndex((element) => element === newTab);
  ...
  useEffect(() => {
    if (data)
      setBgImg(
        data[activeTabIndex]?.cards[0]?.imgUrl ||
          data[activeTabIndex]?.bgImg[theme]
      );
  }, [activeTabIndex, data, theme]);

  useEffect(() => {
    if (searchDropdownValue && data) {
      let newTabIndex = data
        ?.map((_) => _.label)
        ?.findIndex((element) => element === searchDropdownValue);
  ...
  return (
    <div className="max-w-grid p-grid mt-20 md:w-screen text-black dark:text-white min-h-[150vh]">
      <div
        className={`w-full grid mx-auto transition-all ${
          isRightOpen ? "grid-cols-5 " : " grid-cols-1"
        }`}
      >
        <div
          className={`relative transition-all ${
            isRightOpen ? "col-span-5 xl:col-span-4 " : ""
          }`}
        >
          <SearchResultPreview
